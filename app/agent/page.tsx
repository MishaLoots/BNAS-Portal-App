...
export default function AgentPage() {
  ...
  const filteredRows = rows.filter(r => {
    if (search   && !r.event?.toLowerCase().includes(search.toLowerCase()) && !r.artist.name?.toLowerCase().includes(search.toLowerCase())) return false
    if (dateFrom && r.show_date < dateFrom) return false
    if (dateTo   && r.show_date > dateTo)   return false
    if (statusF  && r.status !== statusF)   return false
    if (artistF  && r.artist_id !== artistF) return false
    return true
  })
  const hasFilter = search || dateFrom || dateTo || statusF || artistF
  const uniqueArtists = Array.from(new Map(rows.map(r => [r.artist_id, r.artist])).values())
        ...
        {tab === "shows" && (
          <div className="card p-0">
            ...
            <div className="px-6 py-3 border-b flex flex-wrap gap-3 items-end bg-gray-50">
              ...
              {hasFilter && <button className="text-xs text-gray-400 hover:text-gray-600" onClick={() => { setSearch(""); setDateFrom(""); setDateTo(""); setStatusF(""); setArtistF("") }}>Clear</button>}
              <span className="ml-auto text-xs text-gray-500">{filteredRows.length} of {rows.length} shows</span>
            </div>
            <div className="table-wrap rounded-none rounded-b-xl">
              <table>
                <thead>
                ...
                <tfoot>
                  <tr className="bg-lblue font-semibold">
                    <td colSpan={4}>TOTALS ({filteredRows.length})</td>
                    <td className="text-right font-mono">{ZAR(filteredRows.reduce((s, r) => s + r.gross, 0))}</td>
                    <td className="text-right font-mono">{ZAR(filteredRows.reduce((s, r) => s + r.gross * r.comm_pct, 0))}</td>
                    <td className="text-right font-mono">{ZAR(filteredRows.reduce((s, r) => s + r.gross * r.comm_pct * (1 - (r.artist.bnas_overhead_pct || 0.2)), 0))}</td>
                    <td className="text-right font-mono">{ZAR(filteredRows.reduce((s, r) => s + r.agentEarned, 0))}</td>
                    <td></td>
                  </tr>
                </tfoot>
              </table>
